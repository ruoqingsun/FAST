...
#define TFILE "/Users/ruoqing/Documents/Thesis/Positions/T_file1.txt"
#define KFILE "/Users/ruoqing/Documents/Thesis/Positions/K_file1.txt"
#define IN_TFILE "/Users/ruoqing/Documents/Thesis/Positions/T_file1.txt"
#define IN_KFILE "/Users/ruoqing/Documents/Thesis/Positions/K_file1.txt"
...
void Skinning::add_registration_group()
{
    rebar->TwAddVarRW("register_pose",TW_TYPE_BOOLCPP,&register_pose,
                      " group='Registration' label='Register Pose' help='Register pose from kinect"
                      " off'");
    rebar->TwAddVarRW("in_registration",TW_TYPE_BOOLCPP,&in_registration,
                      " group='Registration' label='In registration' help='in register progress"
                      " off'");
    rebar->TwAddVarRW("file_animation",TW_TYPE_BOOLCPP,&file_animation,
                      " group='Registration' label='Film Animation' help='Film the animation from kinect"
                      " off'");
    
    rebar->TwAddVarRW("load_pose",TW_TYPE_BOOLCPP,&load_pose,
                      " group='Registration' label='Load pose' help='Load transformations from file"
                      " off'");
    ...
    // Collapse group
//    rebar->TwSetParam( "Registration", "opened", TW_PARAM_INT32, 1, &INT_ZERO);
}
...
void Skinning::crowd()
{
    ...
    //for (int i=0; i<numReps; i++) glCallList(display_list_id);
    //glFinish();
    //double sec_end = get_seconds_hires(s);
    //printf("Skinning time = %fms\n", (sec_end - sec_start)*1000.0 / numReps);
...
bool Skinning::read_register_positions()
{
    if(In_T.size()!=0)
        return true;
    
    std::string line;
    if(!In_T_file.is_open())
        In_T_file.open(IN_TFILE);
    
    if(!In_T_file.is_open()){
        cout<<"Cannot read the file "<<IN_TFILE<<endl;
        return false;
    }

    while(std::getline(In_T_file, line))
    {
        MatrixXf tempT;
        tempT.resize(T.rows(), T.cols());
